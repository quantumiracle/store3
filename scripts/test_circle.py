...
from real_circle import load_policy, initialize_camera, close_camera, get_state, get_action
...
def main():
    ...
    for i in range(0, steps*episode):
        ...
        #command_rate.sleep()
    np.savetxt('circle.txt', np.array(results))
    np.save('circle', np.array(results))
    close_camera(camera)
