...
from real_circle import load_policy, initialize_camera, close_camera, get_state, get_action, get_state_threshold, load_policy_threshold, get_ini_pos, get_state_weightedaverage, load_policy_weightedaverage
...
def main():
    ...
    for i in range(0, steps*episode):
        ...
        curr_pos=target_pos
        time.sleep(0.2) # need to make sure the image is captured when tapping
        ...
        results.append([predict, theta])
        print('prediction: ', predict, theta)
        #command_rate.sleep()
    np.savetxt('circle3.txt', np.array(results))
    np.save('circle3', np.array(results))
    close_camera(camera)

def initialize_position(move_controller):
    tap_height=0.02
    ini_position=np.array([0.6, 0.02, -0.068])    # [0.6, 0.02, -0.0005] for small box
