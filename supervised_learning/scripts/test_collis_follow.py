...
from real_collision import load_policy, initialize_camera, close_camera, get_state, get_action
import time
...
def main():
    ...
    for _ in range(5):
        get_state(camera, 0)
    s, collision, xy, original =get_state(camera, 0)
    for i in range(1, episode_length):
        print('collis: ', collision)
        if collision == 0:  # no collision
            colli_pos=np.array([0,0])
        else:  # is at collision
            colli_pos=get_action(policy, s)
            print(i, colli_pos)
        a=np.clip(0.005*colli_pos, -action_range, action_range)
        ax=-a[1]
        ay=a[0]
        a=np.array([ax, ay])
        # plot_list_new_real_arrow(xy, idx=i, action=1000*a, point=colli_pos)
        ...
        time.sleep(0.3)
        s, collision, xy, _=get_state(camera, i, original_contour_centers=original)  # need to make sure the image is captured when tapping
        curr_pos=target_pos
        #command_rate.sleep()
    close_camera(camera)
...
def initialize_position(move_controller):
    tap_height=0.02
    ini_position=np.array([0.6, 0.02, 0.006])    # [0.6, 0.02, -0.0005] for small box
