...
from real_pole import load_policy, initialize_camera, close_camera, get_state, get_action, get_state_threshold, load_policy_threshold, get_ini_pos, get_state_weightedaverage, load_policy_weightedaverage
...
def main():
    ...
    policy=load_policy_weightedaverage() #weighted average    print('Finished Loading Policy!')
    action_range=0.005
    ...
    for i in range(0, steps*episode):
        ...
        target_pos = ini_pos+np.array([a_x,a_y,0])
        pos=np.array([a_x, a_y])
        ...
        predict = policy.predict_one_value(s)[0]
        results.append([predict, pos])
        print('prediction: ', predict, pos)
    np.savetxt('pole3.txt', np.array(results).reshape(-1,4))
    np.save('pole3', np.array(results))
    close_camera(camera)

def initialize_position(move_controller):
    tap_height=0.02
    ini_position=np.array([0.775, -0.09, -0.03])    # [0.6, 0.02, -0.0005] for small box
